...
def drawChessboardCorners(images, reprojected_corner, dist,K_new):
    ...
    for i in range(len(images)):
        ...
        corners = reprojected_corner[i]
        for j in range(len(corners)):
            cv.circle(img, (int(corners[j][0]),int(corners[j][1])), 7, (255,0,255), -1)
        
        x, y, w, h = roi
        img = img[y:y+h, x:x+w]
        # cv.imwrite('caliResult1.png', dst)
        cv.imwrite("Results/" + "undistort" + str(i+1) + ".png", img)
...
def main():
    ...
    ############## UNDISTORTION #####################################################
    # img = cv.imread('/home/ankit/Documents/STUDY/RBE594/HW1/Calibration_Imgs/IMG_20170209_042612.jpg')
    # h,  w = img.shape[:2]
    # newCameraMatrix, roi = cv.getOptimalNewCameraMatrix(K_new, dist, (w,h), 1, (w,h))
    ...
    drawChessboardCorners(images, reprojected_corner, dist,K_new)
    # # crop the image
    # x, y, w, h = roi
    # dst = dst[y:y+h, x:x+w]
    # cv.imwrite('caliResult1.png', dst)
